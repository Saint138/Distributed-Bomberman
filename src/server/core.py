...
from .models import (
    ...
    BLOCK_REGEN_MIN_TIME, BLOCK_REGEN_MAX_TIME, MAX_BLOCKS_ON_MAP,
    MAX_CHAT_MESSAGES, MAX_MESSAGE_LENGTH, DISCONNECT_TIMEOUT
)
...
def add_player(s: State, pid: int):
    x, y = spawn_for(pid)
    s.players[pid] = Player(x=x, y=y)
    print(f"[LOBBY] Player {pid} joined the lobby")
    ...
    for y in range(MAP_HEIGHT):
        for x in range(MAP_WIDTH):
            if x in (0, MAP_WIDTH-1) or y in (0, MAP_HEIGHT-1):
                ...
                m[y][x] = TILE_WALL
            elif (x, y) not in safe and random.random() < 0.2:
                m[y][x] = TILE_BLOCK
    return m
...
def move_player(s: State, pid: int, direction: str):
    if s.game_state != GAME_STATE_PLAYING: return
    if pid not in s.players or not s.players[pid].alive: return
...
def place_bomb(s: State, pid: int):
    ...
    if any(b.x == x and b.y == y for b in s.bombs): return
...
def check_victory(s: State) -> bool:
    if s.game_state != GAME_STATE_PLAYING: return False
